...
class SSOAuth:
    def __init__(self, username=None, password=None):
        ...
        self.session.headers.update({"User-Agent": IPHONE_UA})
                    ...
                    if execution_input:
                        ...
                        if response.status_code in (301, 302, 303, 307, 308):
                            ...
                            if "iclass.buaa.edu.cn" in response.url:
                                ...
                                self._get_user_info(response.url)
                                return True
                            else:
    ...
    def _get_user_info(self, final_url=None):
        """
        Retrieve user information after login.
        Sets self.user_info with user data.

        Args:
            final_url: The final redirect URL from the login flow, which contains loginName.
                       If not provided, falls back to a fresh GET to ICLASS_SERVICE_URL.
        """
        try:
            # Extract loginName from the URL already obtained during login redirect.
            # Avoid an extra GET — the server only embeds loginName in the initial SSO redirect.
            if final_url:
                url_to_search = final_url
                logger.info(f"Extracting login name from redirect URL: {url_to_search}")
            else:
                logger.info("Getting service URL to extract login name...")
                response = self.session.get(config.ICLASS_SERVICE_URL)
                url_to_search = response.url
                logger.info(f"Service URL response: {url_to_search}")

            login_name_match = re.search(r'loginName=([^&#]+)', url_to_search)
            if login_name_match:
